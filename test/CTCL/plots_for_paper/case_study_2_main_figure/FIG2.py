...
_plot_bk_centrality_measures_(p_values_cell_type_squidpy_centralityScores, data, celltype, conditions, scores, subplot_axis_id, legend_loc=legend_loc, title_prefix=title_prefix, title_loc=title_loc, ylabel=ylabel)

# ========== Data for plot D: ==========
celltypes_=['Fibroblasts', 'Macrophages', 'Melanocytes', 'Smooth muscle cells', 'Suprabasal keratinocytes', 'Langerhans cells']
conditions=['AD', 'PSO', 'CTCL']
radii=[1, 2, 3, 4, 5]
cnt=0
for celltype in celltypes_:
    ...
    for condition in conditions:
        data_.append(data[data['condition']==condition])
    data=pd.concat(data_, axis=0)
    _plot_bk_neighborhood_enrichment_(p_values_cell_type_squidpy_nhoodEnrichment, squidpy_nhoodEnrichment_results, data, celltype, conditions, subplot_axis_id, title_prefix=title_prefix, plot_title=plot_title, suptitle=suptitle, ylabel=ylabel)
...
plt.savefig('fig2_subplot_mosaic.pdf', format='pdf', bbox_inches='tight')

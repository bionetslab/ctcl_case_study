...
for count in range(12):
    ...
    if celltypes_of_interest[count]=='T-cells':
        # title=f'{local_heterogeneity_score}\n'
    ...
    # if not(ylabel.endswith('(CTCL vs. AD)')):
    #     title=None
    if count not in [0,3,6,9]:
        title=None
    ...
    if count==3:
        ax=axes[axes_str]
        ax.grid(False)
        ax.text(0.15, 0.35, 'No data shown because\n$P_{subsampled} = 0 = P_{original}$\nfor all subsamples', fontsize=23, color='red')
    else:
        ax = sns.boxplot(**args)
    ...
    if count==4:
        ax=axes[axes_str]
        ax.grid(False)
        ax.text(249.17, 0.330, '$P_{subsampled} = 0$', fontsize=25, color='black', rotation=90)
    if count==5:
        ax=axes[axes_str]
        ax.grid(False)
        ax.text(260.17, 0.330, '$P_{subsampled} = 0$', fontsize=25, color='black', rotation=90)
...
plt.legend(handles=legend_elements, bbox_to_anchor=(-1.70, -0.75), loc='lower left', fontsize=23)
